<template>
  <el-form :model="contract" size="mini" v-if="contract">
    <el-row :gutter="20">
    ...
    <template v-if="contract.tcitemList">
      <el-row
        :gutter="20"
    ...
    <div class="footer">
      <el-button type="primary" @click="upDateContract('1')">提交</el-button>
      <el-button
        type="primary"
        @click="upDateContract('0')"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    emmiter.on('getContractDoubleTime', (times: any) => {
      doubleDatePicker.value = times
    })
    watch(
      doubleDatePicker,
      (newValue) => {
    ...
    const upDateContract = (state: string): void => {
      store.dispatch('contractModule/updateContract', state)
      emit('closeDialog')
    }
